...
class App(object):
    ...
    def scaleImage(app, image, scale, antialias=False):
        ...
        return image.resize(
            (round(image.width * scale), round(image.height * scale)), resample=resample
        )
    ...
    def _redrawAllWrapper(app):
        ...
        app._canvas.create_rectangle(
            0, 0, app.width, app.height, fill="white", width=width, outline=outline
        )
    ...
    def _getEventKeyInfo(event, keysym, char):
        ...
        if (c in [None, ""]) or (len(c) > 1) or (ord(c) > 255):
            key = keysym
            if key.endswith("_L") or key.endswith("_R") or key.endswith("_Lock"):
                key = "Modifier_Key"
    ...
    def _sizeChangedWrapper(app, event=None):
        ...
        app.width, app.height, app.winx, app.winy = [
            int(v) for v in app._root.winfo_geometry().replace("x", "+").split("+")
        ]
    ...
    def run(app):
        ...
        if App._theRoot is None:
            ...
            App._theRoot.bind(
                "<Button-1>", lambda event: App._theRoot.app._mousePressedWrapper(event)
            )
            App._theRoot.bind(
                "<B1-ButtonRelease>",
            ...
            App._theRoot.bind(
                "<KeyPress>", lambda event: App._theRoot.app._keyPressedWrapper(event)
            )
            App._theRoot.bind(
                "<KeyRelease>",
            ...
            App._theRoot.bind(
                "<Configure>", lambda event: App._theRoot.app._sizeChangedWrapper(event)
            )
        else:
