...
class CustomEnvironment(ParallelEnv):
    ...
    def step(self, action: dict[int, Any]):
        ...
        place = None
        if self.terminations[0]:  # agent is removed later
            alive = list(np.unique(self.game.board))
            if -1 in alive:
                alive.remove(-1)
            ...
            reward[0] += (place - self.game.n_players) / (1 - self.game.n_players) - 0.5

        if self.terminations[0]:
            self.agents = []
        return obs, reward, self.terminations, self.truncations, {0: {"place": place}}
