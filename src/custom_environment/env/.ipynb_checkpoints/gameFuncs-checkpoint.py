...
from custom_environment.env.buttonClass import *
import os
...
def gameInit(app):
    ...
    app.minusButton = button(
        (x0 - (y1 - y0), y0, x0, y1),
        fill="#303030",
        outline="#A0A0A0",
        font=("Comic Sans MS", 40, "bold italic"),
    )
    app.plusButton = button(
        (
            x1,
            y0,
            x1 + (y1 - y0),
...
def loadGame(app):
    with open(app.loadFile) as f:
        ...
        for i in range(app.players):
            ...
            app.dict[data[0]] = country(
                data[0], data[1], data[2], data[3], data[4], data[5], attacks
            )
...
def drawBoard(app, canvas):
    ...
    canvas.create_rectangle(
        x0 - cSize, y0 - cSize, x1 + cSize, y1 + cSize, fill="white"
    )  # Map border
...
def drawLeaderBoard(app, canvas):
    ...
    if not (
        app.mouseX > x0 and app.mouseX > y0 and app.mouseX < x1 and app.mouseY < y1
    ):
    ...
    for i in range(len(app.dict)):
        canvas.create_text(
            x - app.width * 0.1, y, fill="white", font=font, text=f"{i + 1}."
        )
        canvas.create_text(
            x, y, fill=tempL[i].color, font=font, text=f"{tempL[i].name}"
        )
        canvas.create_text(
            x + app.width * 0.1, y, fill="white", font=font, text=f"{tempL[i].size}"
        )
        y += 20
...
def drawHud(app, canvas):
    ...
    canvas.create_text(x, y, fill="black", font=font, text=f"Income: {player.size}")
    ...
    canvas.create_rectangle(
        x0,
        y0,
        x0 + (x1 - x0) * player.attackProportion,
        y1,
        fill=sliderColor(player),
        outline="#A0A0A0",
    )
    attackMon = int(player.money * player.attackProportion)
    canvas.create_text(
        (x0 + x1) / 2, (y0 + y1) / 2, text=f"{attackMon}", fill="white", font=font
    )
...
def drawNames(app, canvas):
    ...
    for i in range(len(app.board)):
        for j in range(len(app.board[0])):
            ...
            i1, j1 = (
                int(i + current.maxWidth * current.ratio),
                int(j + current.maxWidth),
            )
            if current.row == -1:
                current.row = i
                current.col = j
            ...
            while (
                i1 < len(app.board)
                and j1 < len(app.board[0])
                and app.board[i][j] == app.board[i1][j1]
                and borders[i1][j1] - borders[i1][j] - borders[i][j1] + borders[i][j]
                == 0
            ):
                current.maxWidth += 1
                current.row = i
...
def drawName(app, current, canvas):
    ...
    fontSize = roundHalfUp(
        widthPerLetter * 0.01 * (app.boardBottomRight[0] - app.boardTopLeft[0])
    )
    if fontSize < 6:
        return
    fill = "white"
    ...
    brightness = (
        int(color[1:3], base=16) + int(color[3:5], base=16) + int(color[5:], base=16)
    ) / 3
    if brightness > 80:
...
def drawAttacks(app, canvas):
    ...
    for i in player.attacks:
        if y1 > app.width / 2:
            break
        if i == -1:
            drawBackgroundAttack(
                app, canvas, app.width / 2 - 75, y0, app.width / 2 + 75, y1
            )
        elif i in app.dict:
            x0 = app.width / 2 - 75
...
def gameMouseDragged(app, event):
    ...
    if event.y < app.width:
        ...
        app.boardTopLeft = (app.boardTopLeft[0] + xShift, app.boardTopLeft[1] + yShift)
        app.boardBottomRight = (
            app.boardBottomRight[0] + xShift,
...
def scale(app, sf):
    topLeftVec = (
        (app.boardTopLeft[0] - app.mouseX) * sf,
        (app.boardTopLeft[1] - app.mouseY) * sf,
    )
    bottomRightVec = (
        (app.boardBottomRight[0] - app.mouseX) * sf,
        (app.boardBottomRight[1] - app.mouseY) * sf,
    )
    return (
        (topLeftVec[0] + app.mouseX, topLeftVec[1] + app.mouseY),
...
def gameKeyPressed(app, event):
    ...
    if (
        event.key == "w"
        and (app.boardBottomRight[0] - app.boardTopLeft[0]) * 1.1 <= app.width * 10
    ):
        app.boardTopLeft, app.boardBottomRight = scale(app, 1.1)
    if (
        event.key == "s"
        and (app.boardBottomRight[0] - app.boardTopLeft[0]) * 0.9 >= app.width * 0.1
    ):
